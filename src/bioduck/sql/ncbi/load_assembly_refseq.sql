...
CREATE OR REPLACE PROCEDURE download_refseq_assembly() AS
BEGIN
    ...
    -- Define file URL
    DECLARE refseq_url VARCHAR := 'https://ftp.ncbi.nlm.nih.gov/genomes/ASSEMBLY_REPORTS/assembly_summary_refseq.txt';
    DECLARE output_file VARCHAR := tmp_dir || '/assembly_summary_refseq.txt';
    
    -- Show status
    SELECT 'Downloading RefSeq assembly data from ' || refseq_url AS status;
    ...
    WHERE file_name = 'assembly_summary_refseq.txt' 
      AND status = 'complete'
    ...
    IF NOT recently_downloaded THEN
        ...
        COPY (SELECT * FROM refseq_url)
        TO output_file;
        
        -- Record the download
        DELETE FROM download_status WHERE file_name = 'assembly_summary_refseq.txt';
        INSERT INTO download_status VALUES 
            ('assembly_summary_refseq.txt', refseq_url, 'complete', CURRENT_TIMESTAMP);
    ELSE
        SELECT 'Using cached RefSeq assembly data (downloaded within the last day)' AS status;
    END IF;
END;

-- Execute the download procedure
CALL download_refseq_assembly();
...
FROM
    read_csv_auto (
        tmp_dir || '/assembly_summary_refseq.txt',
        sep = '\t',
        header = false,
        skip = 2,
